...
func SplitKeyDeposit(key []byte) (proposalID uint64, depositorAddr hmTypes.ThemisAddress) {
	return splitKeyWithAddress(key)
}

// SplitKeyVote split the votes key and returns the proposal id and voter address
func SplitKeyVote(key []byte) (proposalID uint64, voterAddr hmTypes.ThemisAddress) {
	return splitKeyWithAddress(key)
}
...
func splitKeyWithAddress(key []byte) (proposalID uint64, addr hmTypes.ThemisAddress) {
	...
	addr = hmTypes.BytesToThemisAddress(key[9:])
	return
}
