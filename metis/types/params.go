...
const (
	DefaultSprintDuration    uint64 = 10
	DefaultSpanDuration      uint64 = 10 * DefaultSprintDuration
	DefaultFirstSpanDuration uint64 = 100
	DefaultProducerCount     uint64 = 1
)
...
type Params struct {
	...
	SpanDuration   uint64 `json:"span_duration" yaml:"span_duration"`     // span duration ie number of blocks for which val set is frozen on themis
...
func (p Params) Validate() error {
	...
	if err := validateSpanDuration(p.SpanDuration); err != nil {
		return err
	}

	if err := validateProducerCount(p.ProducerCount); err != nil {
		return err
	}
