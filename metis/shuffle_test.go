...
func TestTimeDuration(t *testing.T) {
	t1, err := time.ParseDuration("5m")
	if err != nil {
		t.Fatal(err)
	}

	t.Logf("duration:%v", int64(t1))
	t.Logf("duration seconds:%v", t1.Seconds())
}
...
func TestFloatCmp(t *testing.T) {
	...
	t.Logf("t1:%v, t2:%v, cmp result:%v", t1, t2, t1 > t2)
}
