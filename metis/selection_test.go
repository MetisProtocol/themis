...
func TestCommonHash(t *testing.T) {
	seedHash := sha256Hash(big.NewInt(254).Bytes())

	blkHash := common.BytesToHash(seedHash[:])
	t.Logf("blk hash:%v", blkHash.Hex())

	blkHashByte := blkHash.Bytes()
	seedBytes := helper.ToBytes32(blkHashByte[:32])
