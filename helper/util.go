...
func GetFromAddress(cliCtx context.CLIContext) types.ThemisAddress {
	fromAddress := cliCtx.GetFromAddress()
	if !fromAddress.Empty() {
		return types.AccAddressToThemisAddress(fromAddress)
	}

	return types.BytesToThemisAddress(GetAddress())
}
...
func GetUpdatedValidators(
	...
	for _, v := range validators {
		...
		if val != nil && !validator.IsCurrentValidator(l1Batch) {
			// remove validator
			validator.VotingPower = 0
			updates = append(updates, validator)
		} else if val == nil && validator.IsCurrentValidator(l1Batch) {
			// add validator
			updates = append(updates, validator)
...
func BroadcastMsgsWithCLI(cliCtx context.CLIContext, msgs []sdk.Msg) error {
	txBldr := authTypes.NewTxBuilderFromCLI().WithTxEncoder(GetTxEncoder(cliCtx.Codec)).
		WithGas(MainchainBuildGasLimit)
...
func PrepareTxBuilder(cliCtx context.CLIContext, txBldr authTypes.TxBuilder) (authTypes.TxBuilder, error) {
	...
	// get themis address
	fhAddress := types.BytesToThemisAddress(from)
	...
	// autometisally doing a manual lookup.
...
func populateAccountFromState(txBldr authTypes.TxBuilder, cliCtx context.CLIContext, addr []byte) (authTypes.TxBuilder, error) {
	...
	// key
	key := hmTypes.BytesToThemisAddress(addr)
...
func UnpackSigAndVotes(payload []byte, abi abi.ABI) (votes []byte, sigs []byte, txData []byte, err error) {
	...
	txData = inputDataMap["txData"].([]byte)
...
// GetThemisServerEndpoint returns themis server endpoint
func GetThemisServerEndpoint(endpoint string) string {
	u, _ := url.Parse(GetConfig().ThemisServerURL)
...
func FetchFromAPI(cliCtx cliContext.CLIContext, URL string) (result rest.ResponseWithHeight, err error) {
	...
	if resp.StatusCode == 200 {
		...
		if err := cliCtx.Codec.UnmarshalJSON(body, &result); err != nil {
			return result, err
		}
	...
	return result, fmt.Errorf("error while fetching data from url: %v, status: %v, error: %v", URL, resp.StatusCode, response.Error)
}
