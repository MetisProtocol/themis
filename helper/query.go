...
func GetBeginBlockEvents(client *httpClient.HTTP, height int64) ([]abci.Event, error) {
	...
	if err == nil && blockResults != nil {
		var resultEvent []abci.Event

		// begin block events
		resultEvent = append(resultEvent, blockResults.Results.BeginBlock.GetEvents()...)
		// end block events
		resultEvent = append(resultEvent, blockResults.Results.EndBlock.GetEvents()...)
		// deliver tx events
		for _, tx := range blockResults.Results.DeliverTx {
			resultEvent = append(resultEvent, tx.GetEvents()...)
		}
	...
	for {
		...
		case event := <-eventCh:
			...
			case tmTypes.EventDataNewBlock:
				var resultEvent []abci.Event
				if t.Block.Height == height {
					resultEvent = append(resultEvent, t.ResultBeginBlock.GetEvents()...)
					resultEvent = append(resultEvent, t.ResultEndBlock.GetEvents()...)
					return resultEvent, nil
				}
			case tmTypes.EventDataTx:
				return t.Result.Events, nil
			default:
				return nil, errors.New("timed out waiting for event")
			}
		case <-c.Done():
