...
func TestThemisAppExport(t *testing.T) {
	t.Parallel()

	db := db.NewMemDB()
	happ := NewThemisApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db)
	...
	newHapp := NewThemisApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db)
	_, _, err = newHapp.ExportAppStateAndValidators()
	require.NoError(t, err, "ExportAppStateAndValidators should not have an error")
}

func TestThemisAppExportWithRand(t *testing.T) {
	t.Parallel()
