...
var (
	...
	// DefaultFeeInMetis represents default fee in metis
	DefaultFeeInMetis = big.NewInt(10).Exp(big.NewInt(10), big.NewInt(15), nil)

	// DefaultFeeWantedPerTx fee wanted per tx
	DefaultFeeWantedPerTx = sdk.Coins{sdk.Coin{Denom: authTypes.FeeToken, Amount: sdk.NewIntFromBigInt(DefaultFeeInMetis)}}
)
...
type FeeCollector interface {
	GetModuleAddress(string) types.ThemisAddress
...
func NewAnteHandler(
	...
	return func(ctx sdk.Context, tx sdk.Tx, simulate bool) (newCtx sdk.Context, res sdk.Result, abort bool) {
		if len(tx.GetMsgs()) > 0 {
			ak.Logger(ctx).Info("NewAnteHandler in", "tx", tx.GetMsgs()[0].Type())
		}
		...
		if ctx.ChainID() == helper.MainnetNetwork && ctx.BlockHeight() >= helper.MainnetForkMaxGasHeight {
			gasForTx = helper.MainnetForkMaxGasLimit
		}
		...
		// check if signerAddress is a sequencer
		signerAddr := types.AccAddressToThemisAddress(signerAddrs[0])
		var signerAcc exported.Account

		signerIsValidator := ak.IsValidator(newCtx, sk, signerAddr)
		ak.Logger(ctx).Info("NewAnteHandler check if signer is a validator", "signerAddr", signerAddr.EthAddress().Hex(), "isValidator", signerIsValidator)
		if !signerIsValidator {
			...
			// new account if not exist
			signerAcc = ak.GetAccount(ctx, signerAddr)
			if signerAcc == nil {
				signerAcc = ak.NewAccountWithAddress(newCtx, signerAddr)
			}
			ak.Logger(ctx).Debug("NewAnteHandler is a validator", "newAccount", signerAcc.GetAccountNumber())

			// validator doesn't need deduct fees
		}
