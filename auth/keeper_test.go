...
func (suite *KeeperTestSuite) TestAccountMapperGetSet() {
	...
	addr := hmTypes.BytesToThemisAddress([]byte("some-address"))
...
func (suite *KeeperTestSuite) TestAccountMapperRemoveAccount() {
	...
	addr1 := hmTypes.BytesToThemisAddress([]byte("addr1"))
	addr2 := hmTypes.BytesToThemisAddress([]byte("addr2"))
...
func (suite *KeeperTestSuite) TestGetSequence() {
	...
	_, err = app.AccountKeeper.GetSequence(ctx, hmTypes.BytesToThemisAddress([]byte("addr3")))
	require.Error(t, err)
}
...
func (suite *KeeperTestSuite) TestGetPubKey() {
	...
	addr1 := hmTypes.AccAddressToThemisAddress(addr)
	acc1 := app.AccountKeeper.NewAccountWithAddress(ctx, addr1)
	...
	_, err = app.AccountKeeper.GetPubKey(ctx, hmTypes.BytesToThemisAddress([]byte("addr3")))
	require.Error(t, err)
}
...
func (suite *KeeperTestSuite) TestIterateAccounts() {
	...
	for i := 0; i < newAccounts; i++ {
		addr := hmTypes.BytesToThemisAddress([]byte(fmt.Sprintf("address-%v", i)))
		acc := app.AccountKeeper.NewAccountWithAddress(ctx, addr)
...
func (suite *KeeperTestSuite) TestProposer() {
	...
	require.False(t, ok)
	require.Equal(t, hmTypes.ZeroThemisAddress.Bytes(), proposer.Bytes())
