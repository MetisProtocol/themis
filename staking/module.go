...
import (
	...
	stakingRest "github.com/metis-seq/themis/staking/client/rest"
...
func (AppModuleBasic) VerifyGenesis(bz map[string]json.RawMessage) error {
	...
	stakingManagerAddress := chainManagertData.Params.ChainParams.StakingManagerAddress.EthAddress()
	stakingManagerInstance, _ := contractCaller.GetStakeManagerInstance(stakingManagerAddress)

	// validate validators
	validators := data.Validators
	for _, v := range validators {
		val, err := contractCaller.GetValidatorInfo(v.ID, stakingManagerInstance)
		if err != nil {
			return err
		}
