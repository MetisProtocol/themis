...
// GetCurrentBatch query current l1 batch
func GetCurrentBatch(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "current-batch",
		Short: "show current l1 batch",
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)

			// get current l1 batch
			curerntBatchBytes, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryCurrentL1Batch), nil)
			if err != nil {
				return err
			}

			// check content
			if len(curerntBatchBytes) == 0 {
				fmt.Printf("Current batch not found")
				return nil
			}

			var currentBatch uint64
			if err := json.Unmarshal(curerntBatchBytes, &currentBatch); err != nil {
				return err
			}

			res, err := json.Marshal(map[string]interface{}{"result": currentBatch})
			if err != nil {
				return err
			}
...
// GetAllValSet all validator infos
func GetAllValSet(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "all-validator-set",
		Short: "show all validators",
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)

			// get validator set
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryAllValidatorSet), nil)
			if err != nil {
				return err
			}
...
func IsOldTx(cdc *codec.Codec) *cobra.Command {
	...
	cmd.Flags().String(FlagTxHash, "", "--tx-hash=<tx hash here>")
