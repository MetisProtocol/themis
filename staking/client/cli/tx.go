...
func SendValidatorJoinTx(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "validator-join",
		Short: "Join Themis as a validator",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// get main tx receipt
			receipt, err := contractCallerObj.GetConfirmedTxReceipt(hmTypes.HexToThemisHash(txhash).EthHash(), chainmanagerParams.MainchainTxConfirmations)
			...
			msg := types.NewMsgValidatorJoin(
				...
				viper.GetUint64(FlagActivationBatch),
				amount,
				pubkey,
				hmTypes.HexToThemisHash(txhash),
				logIndex,
	...
	cmd.Flags().Uint64(FlagActivationBatch, 0, "--activation-batch=<activation-batch>")
	...
	if err := cmd.MarkFlagRequired(FlagActivationBatch); err != nil {
		logger.Error("SendValidatorJoinTx | MarkFlagRequired | FlagActivationBatch", "Error", err)
	}
...
func SendValidatorExitTx(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "validator-exit",
		Short: "Exit themis as a validator ",
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			...
			msg := types.NewMsgValidatorExit(
				proposer,
				validator,
				viper.GetUint64(FlagDeactivationBatch),
				hmTypes.HexToThemisHash(txhash),
	...
	cmd.Flags().Uint64(FlagDeactivationBatch, 0, "--deactivation-batch=<deactivation-batch>")
	cmd.Flags().Uint64(FlagBlockNumber, 0, "--block-number=<block-number>")
