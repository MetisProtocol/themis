...
// It represents the current l1 batch
//
//swagger:response currentBatchResponse
type currentBatchResponse struct {
	//in:body
	Output currentBatchStructure `json:"output"`
}
...
type currentBatchStructure struct {
	Height string       `json:"height"`
	Result currentBatch `json:"result"`
}
...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc(
		"/staking/current-batch",
		currentL1BatchHandlerFn(cliCtx),
	).Methods("GET")
	r.HandleFunc(
...
func validatorByAddressHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// error if no found
		if ok := hmRest.ReturnNotFoundIfNoContent(w, res, "No found"); !ok {
			return
		}
...
// swagger:route GET /staking/current-batch staking current L1 batch
// It returns the current  L1 batch
// responses:
//
//	200: currentBatchResponse
//
// get current L1 batch
func currentL1BatchHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cliCtx, ok := rest.ParseQueryHeightOrReturnBadRequest(w, cliCtx, r)
		if !ok {
			return
		...
		currentBatchBytes, height, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryCurrentL1Batch), nil)
		if err != nil {
			RestLogger.Error("Error while fetching current validator set ", "Error", err.Error())
			hmRest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
		...
		// error if no found
		if ok := hmRest.ReturnNotFoundIfNoContent(w, currentBatchBytes, "No found"); !ok {
			return
		}

		var currentBatch uint64
		if err := jsoniter.ConfigFastest.Unmarshal(currentBatchBytes, &currentBatch); err != nil {
			hmRest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
			return
		}

		result, err := jsoniter.ConfigFastest.Marshal(map[string]interface{}{"batch": currentBatch})
		if err != nil {
			RestLogger.Error("Error while marshalling resposne to Json", "error", err)
			hmRest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
