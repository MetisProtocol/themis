...
func (suite *KeeperTestSuite) TestValidator() {
	...
	require.Equal(t, types.HexToThemisAddress(mappedSignerAddress.Hex()), validators[0].Signer, "Signer address doesn't match")
}
...
func (suite *KeeperTestSuite) TestUpdateSigner() {
	...
	newSigner := types.HexToThemisAddress(newPubKey.Address().String())
	...
	require.Equal(t, newSigner, types.HexToThemisAddress(signerAddress.Hex()), "Validator ID should be mapped to Updated Signer Address")
...
func (suite *KeeperTestSuite) TestCurrentValidator() {
	...
	dataItems := []TestDataItem{
		...
		{
			name:        "start batch greater than ackcount",
			startblock:  uint64(3),
			VotingPower: int64(10),
			ackcount:    uint64(1),
			result:      false,
			resultmsg:   "should not be current validator as start batch greater than ackcount.",
		},
	}
	t, app, ctx := suite.T(), suite.app, suite.ctx
	...
	for i, item := range dataItems {
		suite.Run(item.name, func() {
			// Create a Validator [startBatch, endBatch, VotingPower]
			...
			newVal := types.Validator{
				...
				StartBatch:       item.startblock,
				EndBatch:         item.startblock,
				Nonce:            0,
				VotingPower:      item.VotingPower,
				Signer:           types.HexToThemisAddress(pubkey.Address().String()),
				PubKey:           pubkey,
				ProposerPriority: 0,
			}
			...
			stakingKeeper.UpdateL1BatchWithValue(ctx, item.ackcount)
...
func (suite *KeeperTestSuite) TestGetSpanEligibleValidators() {
	...
	// Test ActCount = 0
	app.StakingKeeper.UpdateL1BatchWithValue(ctx, 0)
