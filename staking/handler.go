...
func NewHandler(k Keeper, contractCaller helper.IContractCaller) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) sdk.Result {
		...
		k.Logger(ctx).Info("Staking handle msg", "msgType", msg.Type())
		switch msg := msg.(type) {
		...
		case types.MsgBatchSubmitReward:
			return HandleMsgBatchSubmitReward(ctx, msg, k, contractCaller)
		default:
...
func HandleMsgValidatorJoin(ctx sdk.Context, msg types.MsgValidatorJoin, k Keeper, contractCaller helper.IContractCaller) sdk.Result {
	...
	// TODO:  check if proposer is new validator
...
func HandleMsgBatchSubmitReward(ctx sdk.Context, msg types.MsgBatchSubmitReward, k Keeper, contractCaller helper.IContractCaller) sdk.Result {
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(
			...
			sdk.NewAttribute(sdk.AttributeKeyModule, types.AttributeValueCategory),
			sdk.NewAttribute(types.AttributeKeyBatchID, strconv.FormatUint(msg.BatchID, 10)),
			sdk.NewAttribute(types.AttributeKeyValidatorNonce, strconv.FormatUint(msg.Nonce, 10)),
		),
	})
