...
import (
	...
	protoutils "github.com/metis-seq/themis/proto/utils"
	"github.com/metis-seq/themis/types"
)

func (h *ThemisGRPCServer) Span(ctx context.Context, in *proto.SpanRequest) (*proto.SpanResponse, error) {
	cliCtx := cliContext.NewCLIContext().WithCodec(h.cdc)
	result, err := helper.FetchFromAPI(cliCtx, helper.GetThemisServerEndpoint(fmt.Sprintf(spanURL, in.ID)))
